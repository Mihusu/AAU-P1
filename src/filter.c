...
#define MAX_WORDS 115 //max words in the CV
...
//removes all personal pronouns of the start of every
// sentence of the input section, before calculating cv density
...
void include_section(double *density, char ***sections_out, 
                     int *length, int sections_count, bool *include){
    ...
    while (words < MAX_WORDS && i < sections_count) { 
