...
However, in reality the value of each citizens production value fluctuate. 
Some citizens are hyper-active and produce immense value for there country.
...
\Vref{fig:rate_DK} 
The figure displays the unemployment rate of D a small country with a
generally robust yet agile private business sector and a large public sector.
Some argue that the unemployment numbers are actually higher, as the government
uses subsidies to keep people employed, but even the official statistics clearly
show, that a significant number of people could be looking for a job and
...
Consequently, it is constructive to further discuss unemployment and how it effects both countries and the individual.
However, to make any conclusions on the effects of unemployment it must refer to statistical measurements of unemployment. 
Therefore, the methods used to measure unemployment will also be discussed.
...
As both country and the individual are loose and imprecise terms it is helpful to narrow rhe discussion down to a couple countries and a group of relevant individuals.
...
Furthermore, it is valuable to choose countries which are relatively stable and reliable in there unemployment numbers and statistics so research and findings are constructive.
The two countries that are optimal as candidates are The United States of America and United Kingdom.
Furthermore, USA is the most populated western country, while the UK is a smaller but representative of a European economy , giving contrast within comparably similar socio-economic structured countries. \cite{Economic_similarities_US_Denmark}
...
As unemployment can have almost any imaginable long-term and short-term effect on the unemployed: 
It is productive to focus exclusively on how unemployment directly effects the unemployed´s chances of obtaining a job.
...
But how do we represent unemployment and a nations economy in statistics?
A good estimate of a countries current and future economic state is the growth rate of the GNP(gross national product) and inflation of an economy(this will be further discussed in the following paragraphs).
While a good estimate of the number of unemployed in a country is the unemployment rate (this will also be further discussed in the following paragraphs).
...
To discuss the effects of the unemployment rate on the growth rate it is productive to plot both statistics into the same figure and examine what effects can be seen:

\incfigure{figures/United States Growth Rate (\%) and Unemployment Rate(\%)} \cite{US_Unemployment}\cite{US_Growth_Rate_GNP}

Here we see a clear correlation between growth rate of the GNP and unemployment rate of the US economy.
We see a inverse correlation between the two, meaning that when one decreases the other increases and vice versa.
If we look at the effect of the unemployment rate on the growth rate in the UK it should mirror this conclusion.

\incfigure{figures/United States Growth Rate (\%) and Unemployment Rate(\%)} \cite{UK_Unemployment}\cite{UK_Growth_Rate_GNP}
...
This was expected and supported by Okun´s law, an economic theory that stated that a fall in unemployment would lead to an increase in GNP and therefore would have an inverse effect on the growth rate of the GNP. https://www.investopedia.com/terms/o/okunslaw.asp
...
As inflation is a difficult statistic to accurately define, wage inflation will be used as a representation of inflation in an economy.
